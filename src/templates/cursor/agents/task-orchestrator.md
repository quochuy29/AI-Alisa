...
description: Chuyển approved design (07-design.md) thành execution-ready task list (08-tasks.md). Chạy trong isolated context — chỉ nhận design + spec + context findings, không có conversation history. 3 chiến lược: Sequential, Hierarchical, Adaptive.
tools: Read, Glob, Grep, Write, mcp__context7__resolve-library-id, mcp__context7__get-library-docs, mcp__sequential-thinking__process_thought
model: opus
---
...
**Chạy trong fresh context** — không có conversation history, chỉ có files truyền vào.
...
```
1. Đọc 07-design.md → hiểu toàn bộ scope
2. Đọc 06-requirements-spec.md → map FR→AC
3. Đọc 03-context-findings.md → existing patterns, files liên quan
4. Context7: research libraries cần dùng (nếu chưa biết API mới nhất)
```
...
Sequential Thinking để quyết định chiến lược nếu không rõ.
