...
AskQuestion({
  questions: [{
    ...
    options: [
      { id: "a", label: "🐛 Fix critical/high issues found — /requirements-bug-fix" },
      { id: "b", label: "⚙️ Continue with remaining tasks — /requirements-specs-execute" },
      { id: "c", label: "✅ Archive & close session — /requirements-end" }
    ]
  }]
})
```
→ `"a"` : `/requirements-bug-fix`
→ `"b"` : `/requirements-specs-execute`
→ `"c"` : `/requirements-end`
