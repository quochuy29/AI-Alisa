   ...
   **If `activeBug` is non-null:**
   ...
   AskQuestion({
     questions: [{
       ...
       options: [
         { id: "a", label: "Resolve the bug first — run /requirements-bug-fix" },
   ...
   **If `activeChange` is non-null:**
   ...
   AskQuestion({
     questions: [{
       ...
       options: [
         { id: "a", label: "Resolve the change first — run /requirements-spec-enhance" },
         { id: "b", label: "Archive anyway — mark session as incomplete" },
         { id: "c", label: "Cancel — keep working" }
       ]
     }]
   ...
   ```
   → `"a"` : run `/requirements-spec-enhance`
   → `"b"` : proceed with archive (mark incomplete)
   → `"c"` : do nothing, acknowledge
