...
| Task generation | Prefer fresh Copilot Chat session for 100% clean context |
...
| Convention | Follow `COPILOT.md or CLAUDE.md` / `CLAUDE.md` strictly |
...
### Session Management
- Each `#prompt:requirements-specs-execute` = ONE new Copilot Chat session
- Fresh session after `#prompt:requirements-specs-generate` for tasks (optional but recommended)
...
- `#prompt:requirements-status` — Check current progress
- `#prompt:requirements-current` — View full session details
