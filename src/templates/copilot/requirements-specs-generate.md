...
#prompt:requirements-specs-generate requirements/2025-11-08-1921-feature-name
```
...
2. Read `metadata.json` → determine state:
   ...
   | Specs complete | `specs_complete` | Show error: "use #prompt:requirements-specs-execute" |
...
AskQuestion({
  questions: [{
    ...
    options: [
      ...
      { id: "b", label: "Generate later — start a NEW Copilot Chat session, run #prompt:requirements-specs-generate" }
    ]
  }]
})
```
...
2. Run: #prompt:requirements-specs-generate
...
Ready for execution: #prompt:requirements-specs-execute
(Start a NEW Copilot Chat session for optimal task execution quality)
═══════════════════════════════════════════════════════════════════
```
...
- **No active session** → suggest `#prompt:requirements-status`
- **Incomplete requirements** → show missing files, suggest completing
- **Specs already complete** → "Use #prompt:requirements-specs-execute"
