...
> **GitHub Copilot Usage:** Open Copilot Chat and type `#prompt:requirements-library` or use `@Requirements Manager` and say "show library / query library / initialize library".
...
{
  "version": "1.0",
  "project": "[extracted from folder name or COPILOT.md or CLAUDE.md]",
  "created": "[ISO-8601]",
...
vscode/askQuestion({
  questions: [{
    ...
    options: [
      { id: "a", label: "Scan COPILOT.md or CLAUDE.md / CLAUDE.md — import coding rules" },
...
With sections:
- `## Coding Standards` (from COPILOT.md or CLAUDE.md rules)
...
Library will grow automatically as you complete sessions via #prompt:requirements-end.
To view: #prompt:requirements-library  →  View catalog
```
...
Follow `library-ops protocol` (Query operation) interactively:
...
Invoke `Librarian agent (@librarian)` with:
```yaml
...
2. Follow `library-ops protocol` (Harvest operation) — invoke `Curator agent (@curator)` with that session
3. Show:
   ```
...
- `#prompt:requirements-start` — Library auto-queried during codebase analysis
- `#prompt:requirements-specs-execute` — Library injected into task context
- `#prompt:requirements-end` — Library auto-harvested after session completes
