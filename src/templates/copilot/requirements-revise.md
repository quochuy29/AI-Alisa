...
| **Convention Compliance** | Does code follow `COPILOT.md or CLAUDE.md`/`CLAUDE.md` patterns? |
...
AskQuestion({
  questions: [{
    ...
    options: [
      { id: "a", label: "🐛 Fix drift as bugs — #prompt:requirements-bug-fix" },
      { id: "b", label: "🔄 Add missing items as tasks — #prompt:requirements-spec-enhance" },
      { id: "c", label: "⚙️ Continue with next task — #prompt:requirements-specs-execute" },
...
→ `"b"` : `#prompt:requirements-spec-enhance`
→ `"c"` : `#prompt:requirements-specs-execute`
→ `"d"` : `#prompt:requirements-end`
...
- Files not in directories specified by `COPILOT.md or CLAUDE.md`/`CLAUDE.md`
...
- `#prompt:requirements-bug-fix` — Fix identified drift as bugs
- `#prompt:requirements-spec-enhance` — Add missing items as scope change
- `#prompt:requirements-specs-execute` — Continue implementation
- `#prompt:requirements-code-review` — Full eight-pillar quality review
