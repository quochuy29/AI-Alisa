...
| **Convention Compliance** | Does code follow `CLAUDE.md` patterns? |
...
After showing report, present options using AskUserQuestion tool:
```
AskUserQuestion("Alignment check complete. What would you like to do?", [
  "🐛 Fix drift as bugs — /requirements-bug-fix",
  "🔄 Add missing items as tasks — /requirements-spec-enhance",
  "⚙️ Continue with next task — /requirements-specs-execute",
  "✅ Looks good — archive session"
])
```
→ "🐛 Fix drift…"        : `/requirements-bug-fix`
→ "🔄 Add missing…"      : `/requirements-spec-enhance`
→ "⚙️ Continue…"         : `/requirements-specs-execute`
→ "✅ Looks good…"        : `/requirements-end`
...
- Files not in directories specified by `CLAUDE.md`
